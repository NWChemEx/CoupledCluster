...
void ccsd_e(/* ExecutionContext &ec, */
            ...
            const Tensor<T>& t1, const Tensor<T>& t2, const Tensor<T>& f1, Tensor<T>& chol3d) {
...
void ccsd_t2(/* ExecutionContext& ec, */
             Scheduler& sch, const TiledIndexSpace& MO, const TiledIndexSpace& CI, Tensor<T>& i0,
             const Tensor<T>& t1, Tensor<T>& t2, const Tensor<T>& f1, Tensor<T>& chol3d) {
  auto [cind]   = CI.labels<1>("all");
  ...
  sch.allocate(_a007, _a001_aa, _a001_bb, _a017_aa, _a017_bb, _a006_aa, _a006_bb, _a009_aa,
               _a009_bb, _a021_aa, _a021_bb, _a008_aa, _a008_bb, _a019_aaaa, _a019_abab, _a019_abba,
               _a019_baba, _a019_bbbb, _a020_aaaa, _a020_baba, _a020_abab, _a020_baab, _a020_bbbb,
               _a020_abba, _a022_aaaa, _a022_abab, _a022_bbbb);
  ...
  sch.deallocate(_a007, _a001_aa, _a001_bb, _a017_aa, _a017_bb, _a006_aa, _a006_bb, _a009_aa,
                 _a009_bb, _a021_aa, _a021_bb, _a008_aa, _a008_bb, _a019_aaaa, _a019_abab,
                 _a019_abba, _a019_baba, _a019_bbbb, _a020_aaaa, _a020_baba, _a020_abab, _a020_baab,
                 _a020_bbbb, _a020_abba, _a022_aaaa, _a022_abab, _a022_bbbb //_a022_baba,_a019_baab
  );
...
cd_ccsd_driver_NK(ExecutionContext& ec, const TiledIndexSpace& MO, const TiledIndexSpace& CI,
                  Tensor<T>& d_t1, Tensor<T>& d_t2, Tensor<T>& d_f1, Tensor<T>& d_r1,
                  Tensor<T>& d_r2, std::vector<Tensor<T>>& d_r1s, std::vector<Tensor<T>>& d_r2s,
                  std::vector<Tensor<T>>& d_t1s, std::vector<Tensor<T>>& d_t2s,
                  std::vector<T>& p_evl_sorted, int maxiter, double thresh, double zshiftl,
                  int ndiis, const TAMM_SIZE& noab, Tensor<T>& cv3d, bool writet = false,
  ...
  Scheduler sch{ec};
  // t2_baba
  sch.allocate(t1_aa, t1_bb, t2_aaaa, t2_abab, t2_bbbb, r1_aa, r1_bb, r2_aaaa,
               r2_bbbb, // r2_abab, r2_baba, r2_abba, r2_baab,
               f1_aa_oo, f1_aa_ov, f1_aa_vo, f1_aa_vv, f1_bb_oo, f1_bb_ov, f1_bb_vo, f1_bb_vv,
               chol3d_aa_oo, chol3d_aa_ov, chol3d_aa_vo, chol3d_aa_vv, chol3d_bb_oo, chol3d_bb_ov,
               chol3d_bb_vo, chol3d_bb_vv);
  ...
  if(!ccsd_restart) {
    ...
    // TODO: GMRES uses space allocated for DIIS. So the max
    ...
    double    del       = 1.0e-3; // a small constant for finite difference
    Tensor<T> d_r1_residual{};
    Tensor<T> d_r2_residual{};

    Tensor<T>::allocate(&ec, d_r1_residual, d_r2_residual);

    sch(d_e() = 0)(d_r1_residual() = 0)(d_r2_residual() = 0).execute();

    // TODO: allocate space for GMRES projected problem
    ...
    maxiter = 100; // TODO: make take this out later
    for(int titer = 0; titer < maxiter; titer++) {
      // TODO: save a copy of t1 and t2, this is
      ...
      // TODO: subtract the saved (t1,t2) to recover inv(D)*(r1,r2)
      ...
      // TODO: shouldn't need to calculate the norm of (d_r1, d_r2) again
      ...
      for(int iter = 0; iter < maxgmiter; iter++) {
        ...
        sch(d_t1s[1]() = d_t1s[0]())(d_t2s[1]() = d_t2s[0]()).execute();
        ...
        std::tie(residual, energy) = rest(ec, MO, d_r1s[iter + 1], d_r2s[iter + 1], d_t1s[0],
                                          d_t2s[0], d_e, p_evl_sorted, zshiftl, noab);
      ...
      for(int j = 0; j < maxgmiter + 1; j++) rhs[j] = 0.0;
      rhs[0] = beta;
      nrows  = maxgmiter + 1;
      ncols  = maxgmiter;
      ione   = 1;
      if(maxgmiter > 0) {
        LAPACKE_dgels(LAPACK_COL_MAJOR, 'N', nrows, ncols, ione, Hmat, nrows, rhs, nrows);
      }
  ...
  sch
    .deallocate(t1_aa, t1_bb, t2_aaaa, t2_abab, t2_bbbb, r1_aa, r1_bb, r2_aaaa,
                r2_bbbb, // r2_abab, r2_baba, r2_abba, r2_baab,
                f1_aa_oo, f1_aa_ov, f1_aa_vo, f1_aa_vv, f1_bb_oo, f1_bb_ov, f1_bb_vo, f1_bb_vv,
                chol3d_aa_oo, chol3d_aa_ov, chol3d_aa_vo, chol3d_aa_vv, chol3d_bb_oo, chol3d_bb_ov,
                chol3d_bb_vo, chol3d_bb_vv)

...
void ccsd_t_driver() {
  ...
  if(rank == 0) {
    ...
    if(use_nwc_gpu_kernels)
      cout << "running with nwchem src-centric unfused gpu kernels..." << endl;
    else cout << "running with tensor-gen target-centric unfused gpu kernels..." << endl;
  }

  double ccsd_t_time, total_t_time;
  std::tie(energy1, energy2, ccsd_t_time, total_t_time) = ccsd_t_unfused_driver(
    ec, k_spin, MO1, t_d_t1, t_d_t2, t_d_v2, p_evl_sorted, hf_energy + corr_energy,
    ccsd_options.ngpu, is_restricted, use_nwc_gpu_kernels);
  ...
  if(rank == 0) {
    ...
    //   std::endl; std::cout << std::fixed << "   -> GFLOPS: " << n_gflops << std::endl;

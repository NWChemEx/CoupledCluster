...
ccsd_t_unfused_driver(ExecutionContext& ec, std::vector<int>& k_spin, const TiledIndexSpace& MO,
                      Tensor<T>& d_t1, Tensor<T>& d_t2, Tensor<T>& d_v2,
                      std::vector<T>& k_evl_sorted, double hf_ccsd_energy, int iDevice,
  ...
  if(dev_count_check < iDevice) {
    if(nodezero)
      cout << "ERROR: Please check whether you have " << iDevice
            ...
            for(size_t t_h3b = t_h2b; t_h3b < noab; t_h3b++) {
              if((k_spin[t_p4b] + k_spin[t_p5b] + k_spin[t_p6b]) ==
                ...
                if((!is_restricted) || (k_spin[t_p4b] + k_spin[t_p5b] + k_spin[t_p6b] +
                  ...
                  //     ) {
                  if(next == taskcount) {
                    size_t size = k_range[t_p4b] * k_range[t_p5b] * k_range[t_p6b] *
                                  k_range[t_h1b] * k_range[t_h2b] * k_range[t_h3b];
...
#if defined(USE_CUDA) || defined(USE_HIP)
                      dev_mem_s(k_range[t_h1b], k_range[t_h2b], k_range[t_h3b], k_range[t_p4b],
                                k_range[t_p5b], k_range[t_p6b]);

                      dev_mem_d(k_range[t_h1b], k_range[t_h2b], k_range[t_h3b], k_range[t_p4b],
                                k_range[t_p5b], k_range[t_p6b]);
                    ...
                    ccsd_t_singles_unfused(ec, MO, noab, nvab, k_spin, k_singles, d_t1, d_v2,
                                           k_evl_sorted, k_range, t_h1b, t_h2b, t_h3b, t_p4b, t_p5b,
                                           t_p6b, has_GPU, is_restricted, use_nwc_gpu_kernels);
                    ccsd_t_doubles_unfused(ec, MO, noab, nvab, k_spin, k_doubles, d_t2, d_v2,
                                           k_evl_sorted, k_range, t_h1b, t_h2b, t_h3b, t_p4b, t_p5b,
                                           t_p6b, has_GPU, is_restricted, use_nwc_gpu_kernels);
                            ...
                            for(size_t t_h1 = 0; t_h1 < k_range[t_h1b]; t_h1++)
                              for(size_t t_h2 = 0; t_h2 < k_range[t_h2b]; t_h2++)
                                for(size_t t_h3 = 0; t_h3 < k_range[t_h3b]; t_h3++) {
                                  double denom = (-1 * k_evl_sorted[k_offset[t_p4b] + t_p4] -
...
#if defined(USE_CUDA) || defined(USE_HIP)
                      compute_energy(factor_l, &energy_l[0], &k_evl_sorted[k_offset[t_h1b]],
                                     &k_evl_sorted[k_offset[t_h2b]], &k_evl_sorted[k_offset[t_h3b]],
                                     &k_evl_sorted[k_offset[t_p4b]], &k_evl_sorted[k_offset[t_p5b]],
                                     &k_evl_sorted[k_offset[t_p6b]], k_range[t_h1b], k_range[t_h2b],
                                     k_range[t_h3b], k_range[t_p4b], k_range[t_p5b], k_range[t_p6b],

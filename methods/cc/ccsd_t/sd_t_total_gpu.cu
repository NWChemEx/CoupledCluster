...
*/
// kernel: d1_1
__global__ void kernel_ccsdT_sd1_1(double* t3, double* d_t2_1, double* d_v2_1, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                        ...
                        (blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_h1 +
                         (blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 +
                          (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 +
                           (blk_idx_p4 * FUSION_SIZE_SLICE_2_P4 + idx_p4) * size_p5) *
                            size_p6) *
                           size_h1) *
                          size_h2) *
                         size_h3;
  ...
  for(int i = 0; i < 4; i++)
    for(int j = 0; j < 4; j++) reg_tile[i][j] = 0.0;
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p4 < rng_p4 && idx_h1 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_t2_1[(blk_idx_p4 * FUSION_SIZE_SLICE_2_P4 + idx_p4 +
                  (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + ll +
                   (blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_h1) * size_p5) *
                    size_p4) *
                   size_h7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h2 &&
      ...
      for(int ll = 0; ll < rng_p6; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_2_X] =
          d_v2_1[blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_h3 +
                 (blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_h2 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + ll) * size_h2) *
                   size_h3 +
                 (threadIdx.y + l) * list_stride_v2[6]];
      }
    __syncthreads();
...
// caller: d1_1
void jk_ccsd_t_d1_1(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h1 * size_p5 * size_p4 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p6 * size_h2 * size_h3,
             cudaMemcpyHostToDevice);

  // num_blocks_kernel_1 = CEIL(size_h3, FUSION_SIZE_SLICE_1_H3) * CEIL(size_h2,
  // FUSION_SIZE_SLICE_1_H2) * CEIL(size_h1, FUSION_SIZE_SLICE_1_H1) * CEIL(size_p6,
  // FUSION_SIZE_SLICE_1_P6) * CEIL(size_p5, FUSION_SIZE_SLICE_1_P5) * CEIL(size_p4,
  // FUSION_SIZE_SLICE_1_P4);
  num_blocks_kernel_2 =
    CEIL(size_h3, FUSION_SIZE_SLICE_2_H3) * CEIL(size_h2, FUSION_SIZE_SLICE_2_H2) *
    CEIL(size_h1, FUSION_SIZE_SLICE_2_H1) * CEIL(size_p6, FUSION_SIZE_SLICE_2_P6) *
    CEIL(size_p5, FUSION_SIZE_SLICE_2_P5) * CEIL(size_p4, FUSION_SIZE_SLICE_2_P4);
  ...
  kernel_ccsdT_sd1_1<<<gridsize_2, blocksize_2>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_h7, CEIL(size_h3, FUSION_SIZE_SLICE_2_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_2_H2), CEIL(size_h1, FUSION_SIZE_SLICE_2_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_2_P6), CEIL(size_p5, FUSION_SIZE_SLICE_2_P5),
    CEIL(size_p4, FUSION_SIZE_SLICE_2_P4), str_reg_x_2, str_reg_y_2, size_internal);
#endif

  // Copy the Result from Device to Host
  // cudaMemcpy(host_t3, dev_t3, sizeof(double) * (size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4), cudaMemcpyDeviceToHost);
...
// kernel: d1_2
__global__ void kernel_ccsdT_sd1_2(double* t3, double* d_t2_2, double* d_v2_2, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    // Load Input Tensor to Shared Memory: 16:16
    // # of Internal Indices: 1
    if(idx_p4 < rng_p4 && idx_h1 < rng_h2 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_t2_2[(blk_idx_p4 * FUSION_SIZE_SLICE_2_P4 + idx_p4 +
                  (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + ll +
                   (blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_h1) * size_p5) *
                    size_p4) *
                   size_h7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p6; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_2_X] =
          d_v2_2[(blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_h3 +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h2 +
                   (blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + ll) * size_h1) *
                    size_h3) +
                 (threadIdx.y + l) * list_stride_v2[7]];
      }
    __syncthreads();
...
// caller: d1_2
void jk_ccsd_t_d1_2(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  // int	num_blocks_kernel_1;
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h2 * size_p5 * size_p4 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p6 * size_h1 * size_h3,
             cudaMemcpyHostToDevice);
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_2<<<gridsize_2, blocksize_2>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_h7, CEIL(size_h3, FUSION_SIZE_SLICE_2_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_2_H2), CEIL(size_h1, FUSION_SIZE_SLICE_2_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_2_P6), CEIL(size_p5, FUSION_SIZE_SLICE_2_P5),
...
// kernel: d1_3
__global__ void kernel_ccsdT_sd1_3(double* t3, double* d_t2_3, double* d_v2_3, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    // Load Input Tensor to Shared Memory: 16:16
    // # of Internal Indices: 1
    if(idx_p4 < rng_p4 && idx_h1 < rng_h3 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_t2_3[(blk_idx_p4 * FUSION_SIZE_SLICE_2_P4 + idx_p4 +
                  (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + ll +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_h1) * size_p5) *
                    size_p4) *
                   size_h7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_h3 < rng_h2 && idx_h2 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p6; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_2_X] =
          d_v2_3[(blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_h3 +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_h2 +
                   (blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + ll) * size_h1) *
                    size_h2) +
                 (threadIdx.y + l) * list_stride_v2[8]];
      }
    __syncthreads();
...
// caller: d1_3
void jk_ccsd_t_d1_3(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  // int	num_blocks_kernel_1;
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_p5 * size_p4 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p6 * size_h1 * size_h2,
             cudaMemcpyHostToDevice);
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_3<<<gridsize_2, blocksize_2>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_h7, CEIL(size_h3, FUSION_SIZE_SLICE_2_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_2_H2), CEIL(size_h1, FUSION_SIZE_SLICE_2_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_2_P6), CEIL(size_p5, FUSION_SIZE_SLICE_2_P5),
...
// kernel: d1_4
__global__ void kernel_ccsdT_sd1_4(double* t3, double* d_t2_4, double* d_v2_4, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
  ...
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_p6 && idx_h1 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_4[(blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_p6 +
                   (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h1) * size_p6) *
                    size_p5) *
                   size_h7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h2 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_1_X] =
          d_v2_4[(blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h3 +
                  (blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h2 +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll) * size_h2) *
                    size_h3) +
                 (threadIdx.y + l) * list_stride_v2[0]];
      }
    __syncthreads();
...
// caller: d1_4
void jk_ccsd_t_d1_4(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h1 * size_p6 * size_p5 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p4 * size_h2 * size_h3,
             cudaMemcpyHostToDevice);

  num_blocks_kernel_1 =
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3) * CEIL(size_h2, FUSION_SIZE_SLICE_1_H2) *
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1) * CEIL(size_p6, FUSION_SIZE_SLICE_1_P6) *
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5) * CEIL(size_p4, FUSION_SIZE_SLICE_1_P4);
  // num_blocks_kernel_2 = CEIL(size_h3, FUSION_SIZE_SLICE_2_H3) * CEIL(size_h2,
  // FUSION_SIZE_SLICE_2_H2) * CEIL(size_h1, FUSION_SIZE_SLICE_2_H1) * CEIL(size_p6,
  // FUSION_SIZE_SLICE_2_P6) * CEIL(size_p5, FUSION_SIZE_SLICE_2_P5) * CEIL(size_p4,
  // FUSION_SIZE_SLICE_2_P4);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_4<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
    str_reg_y_1, size_internal);
#endif
  // Copy the Result from Device to Host
  // cudaMemcpy(host_t3, dev_t3, sizeof(double) * (size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4), cudaMemcpyDeviceToHost);
...
// kernel: d1_5
__global__ void kernel_ccsdT_sd1_5(double* t3, double* d_t2_5, double* d_v2_5, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_p6 && idx_h1 < rng_h2 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_5[(blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_p6 +
                   (blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h1) * size_p6) *
                    size_p5) *
                   size_h7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_1_X] =
          d_v2_5[(blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h3 +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h2 +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll) * size_h1) *
                    size_h3) +
                 (threadIdx.y + l) * list_stride_v2[1]];
      }
    __syncthreads();
...
// caller: d1_5
void jk_ccsd_t_d1_5(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h2 * size_p6 * size_p5 * size_h7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h2 * size_p6 * size_p5 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p4 * size_h1 * size_h3,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_5<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
// kernel: d1_6
__global__ void kernel_ccsdT_sd1_6(double* t3, double* d_t2_6, double* d_v2_6, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_p6 && idx_h1 < rng_h3 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_6[(blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_p6 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h1) * size_p6) *
                    size_p5) *
                   size_h7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_h3 < rng_h2 && idx_h2 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_1_X] =
          d_v2_6[(blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h3 +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h2 +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll) * size_h1) *
                    size_h2) +
                 (threadIdx.y + l) * list_stride_v2[2]];
      }
    __syncthreads();
...
// caller: d1_6
void jk_ccsd_t_d1_6(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_p6 * size_p5 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p4 * size_h1 * size_h2,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_6<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
// kernel: d1_7
__global__ void kernel_ccsdT_sd1_7(double* t3, double* d_t2_7, double* d_v2_7, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_p6 && idx_h1 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_7[(blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_p6 +
                   (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h1) * size_p6) *
                    size_p4) *
                   size_h7 +
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h2 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_1_X] =
          d_v2_7[(blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h3 +
                  (blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h2 +
                   (blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll) * size_h2) *
                    size_h3) +
                 (threadIdx.y + l) * list_stride_v2[3]];
      }
    __syncthreads();
...
// caller: d1_7
void jk_ccsd_t_d1_7(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h1 * size_p6 * size_p4 * size_h7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h1 * size_p6 * size_p4 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p5 * size_h2 * size_h3,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_7<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
// kernel: d1_8
__global__ void kernel_ccsdT_sd1_8(double* t3, double* d_t2_8, double* d_v2_8, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_p6 && idx_h1 < rng_h2 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_8[(blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_p6 +
                   (blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h1) * size_p6) *
                    size_p4) *
                   size_h7 +
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_1_X] =
          d_v2_8[(blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h3 +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h2 +
                   (blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll) * size_h1) *
                    size_h3) +
                 (threadIdx.y + l) * list_stride_v2[4]];
      }
    __syncthreads();
...
// caller: d1_8
void jk_ccsd_t_d1_8(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h2 * size_p6 * size_p4 * size_h7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h2 * size_p6 * size_p4 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p5 * size_h1 * size_h3,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_8<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
// kernel: d1_9
__global__ void kernel_ccsdT_sd1_9(double* t3, double* d_t2_9, double* d_v2_9, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_h7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int size_internal) {
...
#pragma unroll 1
  for(int l = 0; l < size_internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_p6 && idx_h1 < rng_h3 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_9[(blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_p6 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h1) * size_p6) *
                    size_p4) *
                   size_h7 +
    ...
    if(idx_h3 < rng_h2 && idx_h2 < rng_h1 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.y][threadIdx.x + ll * FUSION_SIZE_TB_1_X] =
          d_v2_9[(blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h3 +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_h2 +
                   (blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll) * size_h1) *
                    size_h2) +
                 (threadIdx.y + l) * list_stride_v2[5]];
      }
    __syncthreads();
...
// caller: d1_9
void jk_ccsd_t_d1_9(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_h7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_p6 * size_p4 * size_h7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h7 * size_p5 * size_h1 * size_h2,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd1_9<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
*/
// kernel: d2_1
__global__ void kernel_ccsdT_sd2_1(double* t3, double* d_t2_1, double* d_v2_1, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
  // For Shared Memory,
  __shared__ double sm_a[16][64 + 1];
  __shared__ double sm_b[16][64 + 1];
  ...
  int t3_base_thread =
    blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_h3 +
    (blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_h2 +
     (blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_h1 +
      (blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + idx_p6 +
       (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + (blk_idx_p4 * FUSION_SIZE_SLICE_2_P4) * size_p5) *
         size_p6) *
        size_h1) *
       size_h2) *
      size_h3;
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_h1 && idx_h1 < rng_h2 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_1[(blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_p6 +
                   (blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_h1) * size_h1) *
                    size_p4) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_p6 < rng_h3 && idx_h1 < rng_p6 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_v2_1[(blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_p6 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_h1 +
                   (blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll) * size_p6) *
                    size_h3) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
...
// caller: d2_1
void jk_ccsd_t_d2_1(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  // int num_blocks_kernel_2;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h2 * size_h1 * size_p4 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h2 * size_h1 * size_p4 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p5 * size_p6 * size_h3 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  // num_blocks_kernel_2 =   CEIL(size_h3, FUSION_SIZE_SLICE_2_H3) * CEIL(size_h2,
  // FUSION_SIZE_SLICE_2_H2) * CEIL(size_h1, FUSION_SIZE_SLICE_2_H1) *
  //                         CEIL(size_p6, FUSION_SIZE_SLICE_2_P6) * CEIL(size_p5,
  //                         FUSION_SIZE_SLICE_2_P5) * CEIL(size_p4, FUSION_SIZE_SLICE_2_P4);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_1<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
    str_reg_y_1, internal);
#endif
  //
  // cudaMemcpy(host_t3, dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyDeviceToHost);
...
// kernel: d2_2
__global__ void kernel_ccsdT_sd2_2(double* t3, double* d_t2_2, double* d_v2_2, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_h2 && idx_h1 < rng_h3 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_2[(blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll +
                  (blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_p6 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h1) * size_h2) *
                    size_p4) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_p6 < rng_h1 && idx_h1 < rng_p6 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_v2_2[(blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_p6 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_h1 +
                   (blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll) * size_p6) *
                    size_h1) *
                   size_p7 +
...
// caller: d2_2
void jk_ccsd_t_d2_2(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h3 * size_h2 * size_p4 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_h2 * size_p4 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p5 * size_p6 * size_h1 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_2<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
// kernel: d2_3
__global__ void kernel_ccsdT_sd2_3(double* t3, double* d_t2_3, double* d_v2_3, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_h1 && idx_h1 < rng_h3 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_3[(blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_p6 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h1) * size_h1) *
                    size_p4) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
    ...
    if(idx_p6 < rng_h2 && idx_h1 < rng_p6 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_v2_3[(blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_p6 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_h1 +
                   (blk_idx_p5 * FUSION_SIZE_SLICE_1_P5 + ll) * size_p6) *
                    size_h2) *
                   size_p7 +
...
// caller: d2_3
void jk_ccsd_t_d2_3(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h3 * size_h1 * size_p4 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_h1 * size_p4 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p5 * size_p6 * size_h2 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_3<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, FUSION_SIZE_SLICE_1_H3), CEIL(size_h2, FUSION_SIZE_SLICE_1_H2),
    CEIL(size_h1, FUSION_SIZE_SLICE_1_H1), CEIL(size_p6, FUSION_SIZE_SLICE_1_P6),
    CEIL(size_p5, FUSION_SIZE_SLICE_1_P5), CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1,
...
// kernel: d2_4
__global__ void kernel_ccsdT_sd2_4(double* t3, double* d_t2_4, double* d_v2_4, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_h3 && idx_h1 < rng_p6 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_v2_4[(blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_p6 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_h1 +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll) * size_p6) *
                    size_h3) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
...
// caller: d2_4
void jk_ccsd_t_d2_4(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h2 * size_h1 * size_p5 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h2 * size_h1 * size_p5 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p4 * size_p6 * size_h3 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_4<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_p7, CEIL(size_h3, FUSION_SIZE_SLICE_1_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_1_H2), CEIL(size_h1, FUSION_SIZE_SLICE_1_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_1_P6), CEIL(size_p5, FUSION_SIZE_SLICE_1_P5),
    CEIL(size_p4, FUSION_SIZE_SLICE_1_P4), str_reg_x_1, str_reg_y_1, internal);
#endif
  //
  // cudaMemcpy(host_t3, dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyDeviceToHost);
...
// kernel: d2_5
__global__ void kernel_ccsdT_sd2_5(double* t3, double* d_t2_5, double* d_v2_5, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_h1 && idx_h1 < rng_p6 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_v2_5[(blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_p6 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_h1 +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll) * size_p6) *
                    size_h1) *
                   size_p7 +
...
// caller: d2_5
void jk_ccsd_t_d2_5(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h3 * size_h2 * size_p5 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_h2 * size_p5 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p4 * size_p6 * size_h1 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_5<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_p7, CEIL(size_h3, FUSION_SIZE_SLICE_1_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_1_H2), CEIL(size_h1, FUSION_SIZE_SLICE_1_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_1_P6), CEIL(size_p5, FUSION_SIZE_SLICE_1_P5),
...
// kernel: d2_6
__global__ void kernel_ccsdT_sd2_6(double* t3, double* d_t2_6, double* d_v2_6, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    if(idx_p6 < rng_h1 && idx_h1 < rng_h3 &&
      ...
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_t2_6[(blk_idx_p5 * FUSION_SIZE_SLICE_1_P6 + ll +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_1_H1 + idx_p6 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_1_H3 + idx_h1) * size_h1) *
                    size_p5) *
                   size_p7 +
    ...
    if(idx_p6 < rng_h2 && idx_h1 < rng_p6 &&
      ...
      for(int ll = 0; ll < rng_p4; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_1_Y] =
          d_v2_6[(blk_idx_h2 * FUSION_SIZE_SLICE_1_H2 + idx_p6 +
                  (blk_idx_p6 * FUSION_SIZE_SLICE_1_P6 + idx_h1 +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + ll) * size_p6) *
                    size_h2) *
                   size_p7 +
...
// caller: d2_6
void jk_ccsd_t_d2_6(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  int num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h3 * size_h1 * size_p5 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_h1 * size_p5 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p4 * size_p6 * size_h2 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_6<<<gridsize_1, blocksize_1>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_p7, CEIL(size_h3, FUSION_SIZE_SLICE_1_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_1_H2), CEIL(size_h1, FUSION_SIZE_SLICE_1_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_1_P6), CEIL(size_p5, FUSION_SIZE_SLICE_1_P5),
...
// kernel: d2_7
__global__ void kernel_ccsdT_sd2_7(double* t3, double* d_t2_7, double* d_v2_7, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    // Load Input Tensor to Shared Memory: 16:16
    // # of Internal Indices: 1
    if(idx_p4 < rng_h1 && idx_h1 < rng_h2 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p6; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_t2_7[(blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + ll +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_p4 +
                   (blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_h1) * size_h1) *
                    size_p6) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
    ...
    // Load Input Tensor to Shared Memory
    if(idx_p4 < rng_h3 && idx_h1 < rng_p4 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_v2_7[(blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_p4 +
                  (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + ll +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_2_P4 + idx_h1) * size_p5) *
                    size_h3) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
...
// caller: d2_7
void jk_ccsd_t_d2_7(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  // int	num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h2 * size_h1 * size_p6 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h2 * size_h1 * size_p6 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p4 * size_p5 * size_h3 * size_p7,
             cudaMemcpyHostToDevice);

  // num_blocks_kernel_1 =   CEIL(size_h3, FUSION_SIZE_SLICE_1_H3) * CEIL(size_h2,
  // FUSION_SIZE_SLICE_1_H2) * CEIL(size_h1, FUSION_SIZE_SLICE_1_H1) *
  //                         CEIL(size_p6, FUSION_SIZE_SLICE_1_P6) * CEIL(size_p5,
  //                         FUSION_SIZE_SLICE_1_P5) * CEIL(size_p4, FUSION_SIZE_SLICE_1_P4);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_7<<<gridsize_2, blocksize_2>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_p7, CEIL(size_h3, FUSION_SIZE_SLICE_2_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_2_H2), CEIL(size_h1, FUSION_SIZE_SLICE_2_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_2_P6), CEIL(size_p5, FUSION_SIZE_SLICE_2_P5),
    CEIL(size_p4, FUSION_SIZE_SLICE_2_P4), str_reg_x_2, str_reg_y_2, internal);
#endif
  //
  // cudaMemcpy(host_t3, dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyDeviceToHost);
...
// kernel: d2_8
__global__ void kernel_ccsdT_sd2_8(double* t3, double* d_t2_8, double* d_v2_8, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    // Load Input Tensor to Shared Memory: 16:16
    // # of Internal Indices: 1
    if(idx_p4 < rng_h2 && idx_h1 < rng_h3 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p6; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_t2_8[(blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + ll +
                  (blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_p4 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_h1) * size_h2) *
                    size_p6) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
    ...
    // Load Input Tensor to Shared Memory
    if(idx_p4 < rng_h1 && idx_h1 < rng_p4 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_v2_8[(blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_p4 +
                  (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + ll +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_1_P4 + idx_h1) * size_p5) *
                    size_h1) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
...
// caller: d2_8
void jk_ccsd_t_d2_8(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  // int	num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h3 * size_h2 * size_p6 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_h2 * size_p6 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p4 * size_p5 * size_h1 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_8<<<gridsize_2, blocksize_2>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_p7, CEIL(size_h3, FUSION_SIZE_SLICE_2_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_2_H2), CEIL(size_h1, FUSION_SIZE_SLICE_2_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_2_P6), CEIL(size_p5, FUSION_SIZE_SLICE_2_P5),
...
// kernel: d2_9
__global__ void kernel_ccsdT_sd2_9(double* t3, double* d_t2_9, double* d_v2_9, int size_h3,
                                   int size_h2, int size_h1, int size_p6, int size_p5, int size_p4,
                                   int size_p7, int numBlk_h3, int numBlk_h2, int numBlk_h1,
                                   int numBlk_p6, int numBlk_p5, int numBlk_p4, int stride_reg_x,
                                   int stride_reg_y, int internal) {
...
#pragma unroll 1
  for(int l = 0; l < internal; l += FUSION_SIZE_INT_UNIT) {
    ...
    // Load Input Tensor to Shared Memory: 16:16
    // # of Internal Indices: 1
    if(idx_p4 < rng_h1 && idx_h1 < rng_h3 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p6; ll++) {
        sm_a[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_t2_9[(blk_idx_p6 * FUSION_SIZE_SLICE_2_P6 + ll +
                  (blk_idx_h1 * FUSION_SIZE_SLICE_2_H1 + idx_p4 +
                   (blk_idx_h3 * FUSION_SIZE_SLICE_2_H3 + idx_h1) * size_h1) *
                    size_p6) *
                   size_p7 +
                 (threadIdx.x + l)];
      }
    ...
    // Load Input Tensor to Shared Memory
    if(idx_p4 < rng_h2 && idx_h1 < rng_p4 &&
       threadIdx.x < FUSION_SIZE_INT_UNIT - internal_upperbound)
      for(int ll = 0; ll < rng_p5; ll++) {
        sm_b[threadIdx.x][threadIdx.y + ll * FUSION_SIZE_TB_2_Y] =
          d_v2_9[(blk_idx_h2 * FUSION_SIZE_SLICE_2_H2 + idx_p4 +
                  (blk_idx_p5 * FUSION_SIZE_SLICE_2_P5 + ll +
                   (blk_idx_p4 * FUSION_SIZE_SLICE_2_P4 + idx_h1) * size_p5) *
                    size_h2) *
                   size_p7 +
...
// caller: d2_9
void jk_ccsd_t_d2_9(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, size_t size_p7, double* host_t3, double* host_t2,
                    double* host_v2) {
  // # of Blocks for Each Kernel
  // int	 num_blocks_kernel_1;
  ...
  // cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4); cudaMalloc((void**) &dev_t2, sizeof(double) * size_h3 * size_h1 * size_p6 * size_p7);
  ...
  // cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 * size_p5 *
  // size_p4, cudaMemcpyHostToDevice);
  cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h3 * size_h1 * size_p6 * size_p7,
             cudaMemcpyHostToDevice);
  cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_p4 * size_p5 * size_h2 * size_p7,
             cudaMemcpyHostToDevice);
  ...
  dev_t3 = t3_d;
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  kernel_ccsdT_sd2_9<<<gridsize_2, blocksize_2>>>(
    dev_t3, dev_t2, dev_v2, (int) size_h3, (int) size_h2, (int) size_h1, (int) size_p6,
    (int) size_p5, (int) size_p4, (int) size_p7, CEIL(size_h3, FUSION_SIZE_SLICE_2_H3),
    CEIL(size_h2, FUSION_SIZE_SLICE_2_H2), CEIL(size_h1, FUSION_SIZE_SLICE_2_H1),
    CEIL(size_p6, FUSION_SIZE_SLICE_2_P6), CEIL(size_p5, FUSION_SIZE_SLICE_2_P5),
...
// kernel: s1_1
__global__ void jk_ccsd_t_s1_1(double* d_t3, double* d_t2_1, double* d_v2_1, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  //  Shared Memory
  ...
  //  blk-indices
  int blk_idx_p4 =
    blockIdx.x / (num_blks_h3 * num_blks_h2 * num_blks_h1 * num_blks_p6 * num_blks_p5);
  ...
  else rng_h3 = size_h3 % JK_CCSD_T_FUSED_S1_SIZE_TILE_H3;
  ...
  else rng_h2 = size_h2 % JK_CCSD_T_FUSED_S1_SIZE_TILE_H2;
  ...
  else rng_h1 = size_h1 % JK_CCSD_T_FUSED_S1_SIZE_TILE_H1;
  ...
  else rng_p6 = size_p6 % JK_CCSD_T_FUSED_S1_SIZE_TILE_P6;
  ...
  else rng_p5 = size_p5 % JK_CCSD_T_FUSED_S1_SIZE_TILE_P5;
  ...
  else rng_p4 = size_p4 % JK_CCSD_T_FUSED_S1_SIZE_TILE_P4;
                       ...
                       (blk_idx_h1 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H1 + idx_h1 +
                        (blk_idx_p6 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P6 + idx_p6 +
                         (blk_idx_p5 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5 +
                          (blk_idx_p4 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P4) * size_p5) *
                           size_p6) *
                          size_h1) *
                         size_h2) *
                        size_h3;
  ...
  //
  {
    if(idx_h3 < rng_p4 && idx_h2 < rng_h1 && idx_p6 == 0 && idx_h1 == 0)
      sm_a[idx_h3 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P4] =
        d_t2_1[blk_idx_p4 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P4 + idx_h3 +
               (blk_idx_h1 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H1 + idx_h2) * size_p4];

    if(idx_h3 < rng_h3 && idx_h2 < rng_h2 && idx_p6 < rng_p6 && idx_h1 < rng_p5)
      sm_b[idx_h3 + (idx_h2 + (idx_p6 + (idx_h1) *4) * 4) * 4] =
        d_v2_1[blk_idx_h3 * 4 + idx_h3 +
               (blk_idx_h2 * 4 + idx_h2 +
                (blk_idx_p6 * 4 + idx_p6 + (blk_idx_p5 * 4 + idx_h1) * size_p6) * size_h2) *
                 size_h3];
    __syncthreads();
...
// caller: s1_1
void jk_ccsd_t_s1_1(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_1<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
#endif
...
// kernel: s1_2
__global__ void jk_ccsd_t_s1_2(double* d_t3, double* d_t2_2, double* d_v2_2, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  ...
  //
  {
    if(idx_h3 < rng_p4 && idx_h2 < rng_h2 && idx_p6 == 0 && idx_h1 == 0)
      sm_a[idx_h3 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P4] =
        d_t2_2[blk_idx_p4 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P4 + idx_h3 +
               (blk_idx_h2 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H2 + idx_h2) * size_p4];

    if(idx_h3 < rng_h3 && idx_h2 < rng_h1 && idx_p6 < rng_p6 && idx_h1 < rng_p5)
      sm_b[idx_h3 + (idx_h2 + (idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6) *
                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3] =
        d_v2_2[blk_idx_h3 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H3 + idx_h3 +
               (blk_idx_h1 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H1 + idx_h2 +
                (blk_idx_p6 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P6 + idx_p6 +
                 (blk_idx_p5 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5 + idx_h1) * size_p6) *
                  size_h1) *
                 size_h3];
    __syncthreads();
...
// caller:s1_2
void jk_ccsd_t_s1_2(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_2<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_3
__global__ void jk_ccsd_t_s1_3(double* d_t3, double* d_t2_3, double* d_v2_3, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  ...
  //
  {
    if(idx_h3 < rng_p4 && idx_h2 < rng_h3 && idx_p6 == 0 && idx_h1 == 0)
      sm_a[idx_h3 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P4] =
        d_t2_3[blk_idx_p4 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P4 + idx_h3 +
               (blk_idx_h3 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H3 + idx_h2) * size_p4];

    if(idx_h3 < rng_h2 && idx_h2 < rng_h1 && idx_p6 < rng_p6 && idx_h1 < rng_p5)
      sm_b[idx_h3 + (idx_h2 + (idx_p6 + (idx_h1) *4) * 4) * 4] =
        d_v2_3[blk_idx_h2 * 4 + idx_h3 +
               (blk_idx_h1 * 4 + idx_h2 +
                (blk_idx_p6 * 4 + idx_p6 + (blk_idx_p5 * 4 + idx_h1) * size_p6) * size_h1) *
                 size_h2];
    __syncthreads();
...
// caller: s1_3
void jk_ccsd_t_s1_3(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_3<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_4
__global__ void jk_ccsd_t_s1_4(double* d_t3, double* d_t2_4, double* d_v2_4, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  ...
  //
  {
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h2 && idx_p6 < rng_p6 && idx_h1 < rng_p4)
      sm_b[idx_h3 + (idx_h2 + (idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6) *
                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3] =
        d_v2_4[blk_idx_h3 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H3 + idx_h3 +
               (blk_idx_h2 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H2 + idx_h2 +
                (blk_idx_p6 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P6 + idx_p6 +
                 (blk_idx_p4 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P4 + idx_h1) * size_p6) *
                  size_h2) *
                 size_h3];
    __syncthreads();
...
// caller: s1_4
void jk_ccsd_t_s1_4(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_4<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_5
__global__ void jk_ccsd_t_s1_5(double* d_t3, double* d_t2_5, double* d_v2_5, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
...
// caller: s1_5
void jk_ccsd_t_s1_5(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_5<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_6
__global__ void jk_ccsd_t_s1_6(double* d_t3, double* d_t2_6, double* d_v2_6, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
...
// caller: s1_6
void jk_ccsd_t_s1_6(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_6<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_7
__global__ void jk_ccsd_t_s1_7(double* d_t3, double* d_t2_7, double* d_v2_7, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  ...
  //
  {
    ...
    if(idx_h3 < rng_h3 && idx_h2 < rng_h2 && idx_p6 < rng_p5 && idx_h1 < rng_p4)
      sm_b[idx_h3 + (idx_h2 + (idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3] =
        d_v2_7[blk_idx_h3 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H3 + idx_h3 +
               (blk_idx_h2 * JK_CCSD_T_FUSED_S1_SIZE_TILE_H2 + idx_h2 +
                (blk_idx_p5 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5 + idx_p6 +
                 (blk_idx_p4 * JK_CCSD_T_FUSED_S1_SIZE_TILE_P4 + idx_h1) * size_p5) *
                  size_h2) *
                 size_h3];
    __syncthreads();

    //  "p4" x "p5"
    reg_tile[0][0] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (0 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[0][1] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (1 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[0][2] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (2 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[0][3] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (3 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];

    reg_tile[1][0] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (0 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[1][1] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (1 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[1][2] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (2 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[1][3] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (3 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];

    reg_tile[2][0] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (0 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[2][1] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (1 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[2][2] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (2 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[2][3] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (3 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];

    reg_tile[3][0] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (0 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[3][1] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (1 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[3][2] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (2 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[3][3] += sm_a[idx_p6 + (idx_h1) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h2 + (3 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H2) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
  }
...
// caller: s1_7
void jk_ccsd_t_s1_7(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_7<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_8
__global__ void jk_ccsd_t_s1_8(double* d_t3, double* d_t2_8, double* d_v2_8, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  ...
  //
  {
    ...
    //  "p4" x "p5"
    reg_tile[0][0] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (0 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[0][1] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (1 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[0][2] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (2 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[0][3] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (3 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];

    reg_tile[1][0] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (0 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[1][1] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (1 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[1][2] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (2 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[1][3] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (3 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];

    reg_tile[2][0] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (0 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[2][1] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (1 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[2][2] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (2 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[2][3] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (3 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];

    reg_tile[3][0] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (0 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[3][1] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (1 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[3][2] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (2 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
    reg_tile[3][3] -= sm_a[idx_p6 + (idx_h2) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h3 + (idx_h1 + (3 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H3];
  }
...
// caller: s1_8
void jk_ccsd_t_s1_8(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_8<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
// kernel: s1_9
__global__ void jk_ccsd_t_s1_9(double* d_t3, double* d_t2_9, double* d_v2_9, size_t size_h3,
                               size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                               size_t size_p4, size_t num_blks_h3, size_t num_blks_h2,
                               size_t num_blks_h1, size_t num_blks_p6, size_t num_blks_p5,
                               size_t num_blks_p4, size_t stride_reg_x, size_t stride_reg_y) {
  ...
  //
  {
    ...
    //  "p4" x "p5"
    reg_tile[0][0] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (0 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[0][1] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (1 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[0][2] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (2 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[0][3] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (3 + (0) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];

    reg_tile[1][0] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (0 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[1][1] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (1 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[1][2] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (2 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[1][3] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (3 + (1) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];

    reg_tile[2][0] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (0 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[2][1] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (1 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[2][2] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (2 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[2][3] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (3 + (2) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];

    reg_tile[3][0] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (0 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[3][1] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (1 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[3][2] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (2 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
    reg_tile[3][3] += sm_a[idx_p6 + (idx_h3) *JK_CCSD_T_FUSED_S1_SIZE_TILE_P6] *
                      sm_b[idx_h2 + (idx_h1 + (3 + (3) * JK_CCSD_T_FUSED_S1_SIZE_TILE_P5) *
                                                JK_CCSD_T_FUSED_S1_SIZE_TILE_H1) *
                                      JK_CCSD_T_FUSED_S1_SIZE_TILE_H2];
  }
...
// caller: s1_9
void jk_ccsd_t_s1_9(size_t size_h3, size_t size_h2, size_t size_h1, size_t size_p6, size_t size_p5,
                    size_t size_p4, double* host_t3, double* host_t2, double* host_v2) {
  double* dev_t3;
  double* dev_t2;
  double* dev_v2;
...
#ifdef DEBUG_ENALBLE_ALL_KERNEL
  //
  jk_ccsd_t_s1_9<<<gridsize, blocksize>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, num_blks_h3,
    num_blks_h2, num_blks_h1, num_blks_p6, num_blks_p5, num_blks_p4, stride_reg_x, stride_reg_y);
...
#pragma unroll
  for(int i = t_id; i < SINGLE_STAGE_SIZE; i += blockDim.x * blockDim.y) { smem[i] = 0; }
}
...
//------------------------------------------------------------------------------ kernels and callers
//
__global__ void next_unfused_kernel_d1_1(double* dev_t3_d, const double* __restrict__ dev_d1_t2_1,
                                         const double* __restrict__ dev_d1_v2_1, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_h7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
...
#pragma unroll
  ...
  int base_addr_t3 =
    blk_idx_h3 * SIZE_TILE_H3 + idx_h2 +
    (blk_idx_h2 * SIZE_TILE_H2 + idx_h1 +
     (blk_idx_h1 * SIZE_TILE_H1 + idx_h3 +
      (blk_idx_p6 * SIZE_TILE_P6 +
       (blk_idx_p5 * SIZE_TILE_P5 + (blk_idx_p4 * SIZE_TILE_P4 + idx_p6) * size_p5) * size_p6) *
        size_h1) *
       size_h2) *
      size_h3;
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_p4) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) { // p4,h1
        g2s_d1_t2_1<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_1, blk_idx_h1,
                                     idx_h3, blk_idx_p5, size_p5, blk_idx_p4, size_p4, idx_h1,
                                     size_h7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }

      if((idx_h2 < rng_h2) && (idx_p6 < rng_h3) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) { // h3,h2
        g2s_d1_v2_1<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_1, blk_idx_p6,
                                     size_p6, blk_idx_h2, size_h2, idx_h2, blk_idx_h3, size_h3,
                                     idx_p6, threadIdx.y + l_fetch, rng_p6, pipeline);
      }
...
#pragma unroll 4
      for(int idx_reg_x = 0; idx_reg_x < 4; idx_reg_x++) {
        ...
        if(idx_reg_y < rng_p6 && idx_reg_x < rng_p5) {
          dev_t3_d[idx_reg_y * stride_reg_y + idx_reg_x * stride_reg_x] +=
            op_c.reg[idx_reg_x + idx_reg_y * 4];
        }
      }
    }
  }
...
void driver_ccsd_t_d1_1(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  ...
  // cudaMalloc()
  // CUCHK(cudaMalloc((void**) &dev_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4));
  ...
  // cudaMemcpy()
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p4 * size_p5 * size_h1,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h3 * size_h2 * size_p6 * size_h7,
                   cudaMemcpyHostToDevice));

  // Related to Kernels
  // size_t numOperations = 2 * (size_t)(size_h3) * (size_t)(size_h2) * (size_t)(size_h1) *
  ...
  // CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_1,
  // cudaFuncAttributeMaxDynamicSharedMemorySize, maxbytes));
  next_unfused_kernel_d1_1<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    ...
    stride_reg_x, stride_reg_y, size_h7);
...
__global__ void next_unfused_kernel_d1_2(double* dev_t3_d, const double* __restrict__ dev_d1_t2_2,
                                         const double* __restrict__ dev_d1_v2_2,
                                         //
                                         int size_h3, int size_h2, int size_h1, int size_p6,
                                         int size_p5, int size_p4, int size_h7, int numBlk_h3,
                                         int numBlk_h2, int numBlk_h1, int numBlk_p6, int numBlk_p5,
                                         int numBlk_p4, int stride_reg_x, int stride_reg_y,
                                         int size_internal) {
  //
  auto grid  = cooperative_groups::this_grid();
  auto block = cooperative_groups::this_thread_block();
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_p4) &&
        ...
        g2s_d1_t2_2<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_2, blk_idx_h2,
                                     idx_h3, blk_idx_p5, size_p5, blk_idx_p4, size_p4, idx_h1,
                                     size_h7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      ...
      if((idx_h2 < rng_h1) && (idx_p6 < rng_h3) &&
        ...
        g2s_d1_v2_2<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_2, blk_idx_p6,
                                     size_p6, blk_idx_h1, size_h1, idx_h2, blk_idx_h3, size_h3,
                                     idx_p6, threadIdx.y + l_fetch, rng_p6, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d1_2(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p4 * size_p5 * size_h2,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h3 * size_h1 * size_p6 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  // __func__, numTbs); printf ("[%s] Block Size (2D): %2d, %2d\n", __func__, 16, 16); printf ("[%s]
  // # of Operations: %lu\n", __func__, numOperations); printf
  // ("====================================================================================================\n");
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_2, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_2<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
  ...
  // Copy the Result from Device to Host
  // CUCHK(cudaMemcpy(host_t3, dev_t3, sizeof(double) * (size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4), cudaMemcpyDeviceToHost));
...
//
__global__ void next_unfused_kernel_d1_3(double* dev_t3_d, const double* __restrict__ dev_d1_t2_3,
                                         const double* __restrict__ dev_d1_v2_3,
...
#pragma unroll
  ...
  int base_addr_t3 =
    blk_idx_h3 * SIZE_TILE_H3 + idx_h3 +
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h3) && (idx_h1 < rng_p4) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_3<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_3, blk_idx_h3,
                                     idx_h3, blk_idx_p5, size_p5, blk_idx_p4, size_p4, idx_h1,
                                     size_h7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      ...
      if((idx_h2 < rng_h1) && (idx_p6 < rng_h2) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_3<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_3, blk_idx_p6,
                                     size_p6, blk_idx_h1, size_h1, idx_h2, blk_idx_h2, size_h2,
                                     idx_p6, threadIdx.y + l_fetch, rng_p6, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d1_3(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p4 * size_p5 * size_h3,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h2 * size_h1 * size_p6 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_3, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_3<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
//
__global__ void next_unfused_kernel_d1_4(double* dev_t3_d, const double* __restrict__ dev_d1_t2_4,
                                         const double* __restrict__ dev_d1_v2_4, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_h7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
    ...
    (blk_idx_h2 * SIZE_TILE_H2 + idx_h1 +
     (blk_idx_h1 * SIZE_TILE_H1 + idx_h3 +
      (blk_idx_p6 * SIZE_TILE_P6 + idx_p6 +
       (blk_idx_p5 * SIZE_TILE_P5 + (blk_idx_p4 * SIZE_TILE_P4) * size_p5) * size_p6) *
        size_h1) *
       size_h2) *
      size_h3;
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_4<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_4, blk_idx_h1,
                                     idx_h3, blk_idx_p6, size_p6, idx_h1, blk_idx_p5, size_p5,
                                     size_h7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }

      if((idx_h2 < rng_h2) && (idx_p6 < rng_h3) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_4<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_4, blk_idx_p4,
                                     size_p4, blk_idx_h2, size_h2, idx_h2, blk_idx_h3, size_h3,
                                     idx_p6, threadIdx.y + l_fetch, rng_p4, pipeline);
      }
      pipeline.producer_commit();
    }
    pipeline.consumer_wait();
...
void driver_ccsd_t_d1_4(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p5 * size_p6 * size_h1,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h3 * size_h2 * size_p4 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_4, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_4<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
//
__global__ void next_unfused_kernel_d1_5(double* dev_t3_d, const double* __restrict__ dev_d1_t2_5,
                                         const double* __restrict__ dev_d1_v2_5, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_h7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_5<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_5, blk_idx_h2,
                                     idx_h3, blk_idx_p6, size_p6, idx_h1, blk_idx_p5, size_p5,
                                     size_h7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      ...
      if((idx_h2 < rng_h1) && (idx_p6 < rng_h3) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_5<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_5, blk_idx_p4,
                                     size_p4, blk_idx_h1, size_h1, idx_h2, blk_idx_h3, size_h3,
                                     idx_p6, threadIdx.y + l_fetch, rng_p4, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d1_5(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p5 * size_p6 * size_h2,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h3 * size_h1 * size_p4 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_5, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_5<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
__global__ void next_unfused_kernel_d1_6(double* dev_t3_d, const double* __restrict__ dev_d1_t2_6,
                                         const double* __restrict__ dev_d1_v2_6, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_h7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h3) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_6<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_6, blk_idx_h3,
                                     idx_h3, blk_idx_p6, size_p6, idx_h1, blk_idx_p5, size_p5,
                                     size_h7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      ...
      if((idx_h2 < rng_h1) && (idx_p6 < rng_h2) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_6<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_6, blk_idx_p4,
                                     size_p4, blk_idx_h1, size_h1, idx_h2, blk_idx_h2, size_h2,
                                     idx_p6, threadIdx.y + l_fetch, rng_p4, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d1_6(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p5 * size_p6 * size_h3,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h2 * size_h1 * size_p4 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_6, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_6<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
__global__ void next_unfused_kernel_d1_7(double* dev_t3_d, const double* __restrict__ dev_d1_t2_7,
                                         const double* __restrict__ dev_d1_v2_7, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_h7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_7<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_7, blk_idx_h1,
                                     idx_h3, blk_idx_p6, size_p6, idx_h1, blk_idx_p4, size_p4,
                                     size_h7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }

      if((idx_h2 < rng_h2) && (idx_p6 < rng_h3) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_7<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_7, blk_idx_p5,
                                     size_p5, blk_idx_h2, size_h2, idx_h2, blk_idx_h3, size_h3,
                                     idx_p6, threadIdx.y + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
    pipeline.consumer_wait();
...
void driver_ccsd_t_d1_7(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p4 * size_p6 * size_h1,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h3 * size_h2 * size_p5 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_7, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_7<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
__global__ void next_unfused_kernel_d1_8(double* dev_t3_d, const double* __restrict__ dev_d1_t2_8,
                                         const double* __restrict__ dev_d1_v2_8, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_h7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_8<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_8, blk_idx_h2,
                                     idx_h3, blk_idx_p6, size_p6, idx_h1, blk_idx_p4, size_p4,
                                     size_h7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }
      ...
      if((idx_h2 < rng_h1) && (idx_p6 < rng_h3) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_8<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_8, blk_idx_p5,
                                     size_p5, blk_idx_h1, size_h1, idx_h2, blk_idx_h3, size_h3,
                                     idx_p6, threadIdx.y + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d1_8(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p4 * size_p6 * size_h2,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h3 * size_h1 * size_p5 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_8, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_8<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
__global__ void next_unfused_kernel_d1_9(double* dev_t3_d, const double* __restrict__ dev_d1_t2_9,
                                         const double* __restrict__ dev_d1_v2_9,
                                         //
                                         int size_h3, int size_h2, int size_h1, int size_p6,
                                         int size_p5, int size_p4, int size_h7, int numBlk_h3,
                                         int numBlk_h2, int numBlk_h1, int numBlk_p6, int numBlk_p5,
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h3) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_t2_9<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d1_t2_9, blk_idx_h3,
                                     idx_h3, blk_idx_p6, size_p6, idx_h1, blk_idx_p4, size_p4,
                                     size_h7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }
      ...
      if((idx_h2 < rng_h1) && (idx_p6 < rng_h2) &&
         threadIdx.y < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d1_v2_9<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d1_v2_9, blk_idx_p5,
                                     size_p5, blk_idx_h1, size_h1, idx_h2, blk_idx_h2, size_h2,
                                     idx_p6, threadIdx.y + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d1_9(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_h7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  // CUCHK(cudaMemcpy(dev_t3, host_t3, sizeof(double) * size_h3 * size_h2 * size_h1 * size_p6 *
  // size_p5 * size_p4, cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_t2, host_t2, sizeof(double) * size_h7 * size_p4 * size_p6 * size_h3,
                   cudaMemcpyHostToDevice));
  CUCHK(cudaMemcpy(dev_v2, host_v2, sizeof(double) * size_h2 * size_h1 * size_p5 * size_h7,
                   cudaMemcpyHostToDevice));
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d1_9, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d1_9<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_h7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_h7);
...
///
///
__global__ void next_unfused_kernel_d2_1(double* dev_t3_d, const double* __restrict__ dev_d2_t2_1,
                                         const double* __restrict__ dev_d2_v2_1, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
  //
  auto grid  = cooperative_groups::this_grid();
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_h2) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_1<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_1, blk_idx_h2,
                                     idx_h1, blk_idx_h1, size_h1, idx_h3, blk_idx_p4, size_p4,
                                     size_p7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }

      if((idx_h3 < rng_h3) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_1<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_1, blk_idx_p5,
...
void driver_ccsd_t_d2_1(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
               CEIL(size_h1, SIZE_TILE_H1) * CEIL(size_p6, SIZE_TILE_P6) *
  ...
  // Related to Kernels
  // size_t numOperations = 2 * (size_t)(size_h3) * (size_t)(size_h2) * (size_t)(size_h1) *
  // (size_t)(size_p6) * (size_t)(size_p5) * (size_t)(size_p4) * (size_t)(size_p7);
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_1, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_1<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
  cudaDeviceSynchronize();
  CUCHK(cudaGetLastError());
...
__global__ void next_unfused_kernel_d2_2(double* dev_t3_d, const double* __restrict__ dev_d2_t2_2,
                                         const double* __restrict__ dev_d2_v2_2, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_h3) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_2<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_2, blk_idx_h3,
                                     idx_h1, blk_idx_h2, size_h2, idx_h3, blk_idx_p4, size_p4,
                                     size_p7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }

      if((idx_h3 < rng_h1) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_2<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_2, blk_idx_p5,
                                     blk_idx_p6, size_p6, idx_h1, blk_idx_h1, size_h1, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d2_2(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_2, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_2<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_3(double* dev_t3_d, const double* __restrict__ dev_d2_t2_3,
                                         const double* __restrict__ dev_d2_v2_3, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_h3) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_3<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_3, blk_idx_h3,
                                     idx_h1, blk_idx_h1, size_h1, idx_h3, blk_idx_p4, size_p4,
                                     size_p7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_3<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_3, blk_idx_p5,
                                     blk_idx_p6, size_p6, idx_h1, blk_idx_h2, size_h2, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d2_3(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_3, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_3<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_4(double* dev_t3_d, const double* __restrict__ dev_d2_t2_4,
                                         const double* __restrict__ dev_d2_v2_4, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_h2) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_4<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_4, blk_idx_h2,
                                     idx_h1, blk_idx_h1, size_h1, idx_h3, blk_idx_p5,
                                     size_p5, // reg_y: p5
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
...
void driver_ccsd_t_d2_4(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_4, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_4<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_5(double* dev_t3_d, const double* __restrict__ dev_d2_t2_5,
                                         const double* __restrict__ dev_d2_v2_5, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_h3) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_5<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_5, blk_idx_h3,
                                     idx_h1, blk_idx_h2, size_h2, idx_h3, blk_idx_p5, size_p5,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }

      if((idx_h3 < rng_h1) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_5<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_5, blk_idx_p4,
                                     blk_idx_p6, size_p6, idx_h1, blk_idx_h1, size_h1, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }
      pipeline.producer_commit();
...
void driver_ccsd_t_d2_5(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_5, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_5<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_6(double* dev_t3_d, const double* __restrict__ dev_d2_t2_6,
                                         const double* __restrict__ dev_d2_v2_6, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_h3) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_6<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_6, blk_idx_h3,
                                     idx_h1, blk_idx_h1, size_h1, idx_h3, blk_idx_p5, size_p5,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }

      if((idx_h3 < rng_h2) && (idx_h1 < rng_p6) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_6<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_6, blk_idx_p4,
                                     blk_idx_p6, size_p6, idx_h1, blk_idx_h2, size_h2, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p4, pipeline);
      }
      pipeline.producer_commit();
...
void driver_ccsd_t_d2_6(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_6, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_6<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_7(double* dev_t3_d, const double* __restrict__ dev_d2_t2_7,
                                         const double* __restrict__ dev_d2_v2_7, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_h2) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_7<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_7, blk_idx_h2,
                                     idx_h1, blk_idx_h1, size_h1, idx_h3, blk_idx_p6, size_p6,
                                     size_p7, threadIdx.x + l_fetch, rng_p6, pipeline);
      }

      if((idx_h3 < rng_h3) && (idx_h1 < rng_p4) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_7<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_7, blk_idx_p4,
                                     idx_h1, blk_idx_p5, size_p5, blk_idx_h3, size_h3, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d2_7(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_7, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_7<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_8(double* dev_t3_d, const double* __restrict__ dev_d2_t2_8,
                                         const double* __restrict__ dev_d2_v2_8, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_h3) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_8<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_8, blk_idx_h3,
                                     idx_h1, blk_idx_h2, size_h2, idx_h3, blk_idx_p6, size_p6,
                                     size_p7, threadIdx.x + l_fetch, rng_p6, pipeline);
      }

      if((idx_h3 < rng_h1) && (idx_h1 < rng_p4) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_8<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_8, blk_idx_p4,
                                     idx_h1, blk_idx_p5, size_p5, blk_idx_h1, size_h1, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d2_8(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_8, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_8<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);
...
__global__ void next_unfused_kernel_d2_9(double* dev_t3_d, const double* __restrict__ dev_d2_t2_9,
                                         const double* __restrict__ dev_d2_v2_9, int size_h3,
                                         int size_h2, int size_h1, int size_p6, int size_p5,
                                         int size_p4, int size_p7, int numBlk_h3, int numBlk_h2,
                                         int numBlk_h1, int numBlk_p6, int numBlk_p5, int numBlk_p4,
                                         int stride_reg_x, int stride_reg_y, int size_internal) {
...
#pragma unroll 1
    for(; fetch_batch < num_batches && fetch_batch < (compute_batch + NUM_STAGE); ++fetch_batch) {
      ...
      if((idx_h3 < rng_h1) && (idx_h1 < rng_h3) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_t2_9<lda, 1, 4 * lda>(sm_a + STAGE_OFFSET * shared_idx, dev_d2_t2_9, blk_idx_h3,
                                     idx_h1, blk_idx_h1, size_h1, idx_h3, blk_idx_p6, size_p6,
                                     size_p7, threadIdx.x + l_fetch, rng_p6, pipeline);
      }
      ...
      if((idx_h3 < rng_h2) && (idx_h1 < rng_p4) &&
         threadIdx.x < SIZE_UNIT_INT - internal_upperbound) {
        g2s_d2_v2_9<lda, 1, 4 * lda>(sm_b + STAGE_OFFSET * shared_idx, dev_d2_v2_9, blk_idx_p4,
                                     idx_h1, blk_idx_p5, size_p5, blk_idx_h2, size_h2, idx_h3,
                                     size_p7, threadIdx.x + l_fetch, rng_p5, pipeline);
      }
      pipeline.producer_commit();
    }
...
void driver_ccsd_t_d2_9(int size_h3, int size_h2, int size_h1, int size_p6, int size_p5,
                        int size_p4, int size_p7, double* host_t3, double* host_t2,
                        double* host_v2) {
  //
  int numTbs = CEIL(size_h3, SIZE_TILE_H3) * CEIL(size_h2, SIZE_TILE_H2) *
  ...
  //
  int maxbytes = 98304; // 96 KB
  CUCHK(cudaFuncSetAttribute(next_unfused_kernel_d2_9, cudaFuncAttributeMaxDynamicSharedMemorySize,
                             maxbytes));
  next_unfused_kernel_d2_9<<<gridsize_1, blocksize_1, 2 * NUM_STAGE * 8 * STAGE_OFFSET, 0>>>(
    dev_t3, dev_t2, dev_v2, size_h3, size_h2, size_h1, size_p6, size_p5, size_p4, size_p7,
    CEIL(size_h3, SIZE_TILE_H3), CEIL(size_h2, SIZE_TILE_H2), CEIL(size_h1, SIZE_TILE_H1),
    CEIL(size_p6, SIZE_TILE_P6), CEIL(size_p5, SIZE_TILE_P5), CEIL(size_p4, SIZE_TILE_P4),
    stride_reg_x, stride_reg_y, size_p7);

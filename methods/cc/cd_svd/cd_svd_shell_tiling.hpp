...
Tensor<TensorType> cd_svd(SystemData& sys_data, ExecutionContext& ec, TiledIndexSpace& tMO,
...
#if !defined(USE_UPCXX)
  Tensor<TensorType> g_d_tamm{tAOt, tAOt};
  Tensor<TensorType> g_r_tamm{tAOt, tAOt};
  Tensor<TensorType> g_chol_tamm{tAOt, tAOt, tCI};
  g_r_tamm.set_dense();
  g_d_tamm.set_dense();
  ...
  */
  ...
  auto compute_diagonals = [&](const IndexVector& blockid) {
    ...
    std::vector<TensorType> dbuf(n1 * n2);
      ...
      for(size_t f3 = 0; f3 != n3; ++f3) {
        ...
        for(size_t f4 = 0; f4 != n4; ++f4) {
          ...
          for(auto icount = 0U; icount != count; ++icount) {
            rbuf[f3 * n4 + f4] -= cbuf[icount + f4 * bd1 + f3 * n4 * bd1] * k_row[icount];
          }
          auto vtmp                              = rbuf[f3 * n4 + f4] / sqrt(val_d0);
          cbuf[count + f4 * bd1 + f3 * n4 * bd1] = vtmp;
          dbuf[f3 * n4 + f4] -= vtmp * vtmp;
        }
      }

...
Tensor<TensorType> cd_svd_ao(ExecutionContext& ec, TiledIndexSpace& tMO, TiledIndexSpace& tAOt,
                             const TAMM_SIZE ndocc, const TAMM_SIZE nao,
                             const TAMM_SIZE freeze_core, const TAMM_SIZE freeze_virtual,
                             Tensor<TensorType> C_AO, Tensor<TensorType> F_AO,
                             Tensor<TensorType> F_MO, TAMM_SIZE& chol_count,
                             const tamm::Tile max_cvecs, double diagtol,
  ...
  */
  ...
  auto compute_diagonals = [&](const IndexVector& blockid) {
    ...
    if(buf_1212 == nullptr) return; // if all integrals screened out, skip to next quartet
  ...
  do {
    ...
    if(rank == 0 && debug) {
      cout << "time for computing delems = "
           << std::chrono::duration_cast<std::chrono::duration<double>>((init_t2 - init_t1)).count()
           << "secs" << endl;
    }
    ...
    auto update_columns = [&](const IndexVector& blockid) {
      ...
      if(rank == 0 && debug) {
        cout << "time for get chol-vuv = "
             << std::chrono::duration_cast<std::chrono::duration<double>>((get_t2 - get_t1)).count()
             << "secs" << endl;
      }
      ...
      if(buf_3412 == nullptr) return; // if all integrals screened out, skip to next quartet
      ...
      if(rank == 0 && debug) {
        cout
          << "loop time = "
          << std::chrono::duration_cast<std::chrono::duration<double>>((loop_t2 - loop_t1)).count()
          << "secs" << endl;
      }
      ...
      if(rank == 0 && debug) {
        cout << "time for put chol-vuv = "
             << std::chrono::duration_cast<std::chrono::duration<double>>((put_t2 - put_t1)).count()
             << "secs" << endl;
      }
    };
    ...
    // cout << "max: (" << bfu << bfv << "|" << bfu << bfv << ") = " << max << " " << sqrt(max) <<
    // endl; cout << "shells: " << bf2shell(bfu) << " " << bf2shell(bfv) << endl;
  ...
  Scheduler{ec}(CholVpv_tamm(cindexp, pmo, mup) =
                  CTiled_tamm(nup, pmo) * CholVuv_opt(cindexp, nup, mup))
    .execute();
  ...
  Tensor<TensorType> CholVpr_tamm{{tCIp, tMO, tMO},
                                  {SpinPosition::ignore, SpinPosition::upper, SpinPosition::lower}};
  Tensor<TensorType>::allocate(&ec, CholVpr_tamm);
  Scheduler{ec}(CholVpr_tamm(cindexp, pmo, rmo) +=
                CTiled_tamm(mup, rmo) * CholVpv_tamm(cindexp, pmo, mup))
    .execute();
  ...
  //         // SVD: CholVpr(p, r, icount) +=
  //         Vpsigma(p,fs,icount)*Vpsigma(r,fs,icount)*evs.at(icount).second(fs);
  //       }
  //     }
  //   }
  // }

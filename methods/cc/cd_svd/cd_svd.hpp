...
Tensor<TensorType> cd_svd(ExecutionContext& ec, TiledIndexSpace& tMO, TiledIndexSpace& tAO,
                          ...
                          const TAMM_SIZE freeze_virtual, Tensor<TensorType> C_AO,
                          Tensor<TensorType> F_AO, Tensor<TensorType> F_MO, TAMM_SIZE& chol_count,
                          const tamm::Tile max_cvecs, double diagtol, libint2::BasisSet& shells,
  ...
  */
  ...
  do {
    ...
    // cout << "new update cols --- s1,s2,n1,n2 = "  << s1 << "," << s2 << "," << n1 <<"," << n2
    // <<endl;
    ...
    for(auto icount = 0U; icount != count; ++icount) {
      delems[icount] = cvuvbuf[(icount * block_dims_vuv[1] + bfu) * block_dims_vuv[2] + bfv];
    }
    ...
    auto update_columns = [&](const IndexVector& blockid) {
      ...
      for(Index s3 = s3range_start; s3 <= s3range_end; ++s3) {
        ...
        for(Index s4 = s4range_start; s4 <= s4range_end; ++s4) {
          ...
          //     for (auto icount = 0U; icount != count; ++icount) {
          //       x -=
          //       vuvbuf[(icount*bd0+f3+curshelloffset_i)*bd1+f4+curshelloffset_j]*delems[icount];
          //     }
          ...
          for(size_t i = curshelloffset_i; i < dimi; i++) {
            for(size_t j = curshelloffset_j; j < dimj; j++) {
              ...
              auto x     = buf_3412[f3412];
              for(auto icount = 0U; icount != count; ++icount) {
                x -= vuvbuf[(icount * bd0 + f3 + curshelloffset_i) * bd1 + f4 + curshelloffset_j] *
                     delems[icount];
              }
      ...
      l_time = std::chrono::duration_cast<std::chrono::duration<double>>((l_t2 - l_t1)).count();
    ...
    // if(rank == 0 && debug) {
    //   std::cout << "----------Time taken for iter " << citer << ": " << cd_time << "
    //   secs---------\n"; std::cout << "Time taken for puts : " << put_time << " secs\n"; std::cout
    //   << "Time taken for gets: " << get_time << " secs\n";
    // }
    // if(count<=2) {
  ...
  auto lambdacv = [&](const IndexVector& bid) {
    ...
    for(auto i = 0U; i < dsize; i++) dbuf[i] = sbuf[i];
  ...
  Scheduler{ec}(CholVpv_tamm(cindexp, pmo, mu) =
                  CTiled_tamm(nu, pmo) * CholVuv_opt(cindexp, nu, mu))
    .execute();
  ...
  Scheduler{ec}(CholVpr_tamm(cindexp, pmo, rmo) +=
                CTiled_tamm(mu, rmo) * CholVpv_tamm(cindexp, pmo, mu))
    .execute();

...
void two_index_transform(SystemData sys_data, ExecutionContext& ec,
                         Tensor<TensorType> C_alpha_AO,
                         Tensor<TensorType> F_alpha_AO,
                         Tensor<TensorType> C_beta_AO,
                         Tensor<TensorType> F_beta_AO, Tensor<TensorType> F_MO,
                         Tensor<TensorType> lcao, bool isdlpno = false) {
    ...
    if(rank == 0) {
        ...
        cout << std::endl
             << "nAO, nMO, nelectrons = " << nao << ", " << N << ", "
             << n_occ_alpha + n_occ_beta << endl;
        ...
        if(!isdlpno) {
            ...
            cout << "n_occ_alpha, n_vir_alpha, n_occ_beta, n_vir_beta = "
                 << n_occ_alpha << "," << n_vir_alpha << "," << n_occ_beta
                 << "," << n_vir_beta << endl;
            ...
            Matrix F_MO_alpha =
              C_alpha_eig.transpose() * (F_alpha_AO_eig * C_alpha_eig);
            Matrix F_MO_beta = F_MO_alpha;
            if(is_uhf)
                F_MO_beta =
                  C_beta_eig.transpose() * (F_beta_AO_eig * C_beta_eig);

            Matrix F;
            F.setZero(N, N);
            /*
              F_MO_alpha = | F_MO_alpha_oo, F_MO_alpha_ov |   F_MO_beta = |
              F_MO_beta_oo, F_MO_beta_ov | | F_MO_alpha_vo, F_MO_alpha_vv | |
              F_MO_beta_vo, F_MO_beta_vv |

              F_MO       = |  F_MO_alpha_oo           0 F_MO_alpha_ov 0 | | 0
              F_MO_beta_oo            0             F_MO_beta_ov   | |
    ...
    if(rank == 0) {
        std::cout << std::endl
                  << "Time taken for Fao->Fmo transform: " << hf_time << " secs"
                  << endl;
        cout << std::endl
             << "-----------------------------------------------------" << endl;
    }

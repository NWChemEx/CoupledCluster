  ...
  if(rank == 0) {
    ...
    if(!isdlpno) {
      ...
      */
      ...
      F.block(0, nmo_occ, n_occ_alpha, n_vir_alpha) =
        F_MO_alpha.block(0, n_occ_alpha, n_occ_alpha, n_vir_alpha);
      F.block(nmo_occ, 0, n_vir_alpha, n_occ_alpha) =
        F_MO_alpha.block(n_occ_alpha, 0, n_vir_alpha, n_occ_alpha);
      F.block(nmo_occ, nmo_occ, n_vir_alpha, n_vir_alpha) =
        F_MO_alpha.block(n_occ_alpha, n_occ_alpha, n_vir_alpha, n_vir_alpha);

      F.block(n_occ_alpha, n_occ_alpha, n_occ_beta, n_occ_beta) =
        F_MO_beta.block(0, 0, n_occ_beta, n_occ_beta);
      F.block(n_occ_alpha, nmo_occ + n_vir_alpha, n_occ_beta, n_vir_beta) =
        F_MO_beta.block(0, n_occ_beta, n_occ_beta, n_vir_beta);
      F.block(nmo_occ + n_vir_alpha, n_occ_alpha, n_vir_beta, n_occ_beta) =
        F_MO_beta.block(n_occ_beta, 0, n_vir_beta, n_occ_beta);
      F.block(nmo_occ + n_vir_alpha, nmo_occ + n_vir_alpha, n_vir_beta, n_vir_beta) =
        F_MO_beta.block(n_occ_beta, n_occ_beta, n_vir_beta, n_vir_beta);

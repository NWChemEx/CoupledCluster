...
Tensor<TensorType> cd_svd_ga(SystemData& sys_data, ExecutionContext& ec, TiledIndexSpace& tMO,
  ...
  if(iproc < cd_nranks) { // throttle
    ...
    if(iproc == 0)
      std::cout << std::endl << "Time taken for cholesky decomp: " << hf_time << " secs" << endl;
...
#define DO_SVD 0
    ...
    for(decltype(count) kk = 0; kk < count; kk++) {
      if(next == taskcount) {
        ...
        // TODO
        LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'L', (BLA_LAPACK_INT) nbf, &k_ij[0],
                       (BLA_LAPACK_INT) nbf, &k_eval_r[0]);
        ...
        blas::gemm(blas::Layout::RowMajor, blas::Op::NoTrans, blas::Op::Trans, N, N, m, 1, &k_pi[0],
                   N, &k_qj[0], N, 0, &k_pq[0], N);

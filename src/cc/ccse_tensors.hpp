...
public:
  ...
  CCSE_Tensors(const TiledIndexSpace& MO, TiledIndexSpaceVec tis, std::string tensor_name,
    ...
    for(size_t x = 0; x < tis.size(); x++) {
      if(tis[x] != O && tis[x] != V) {
        if(ndims == 3)
          is_mo_3d = false; // assuming only 3D tensors have an independent index space
        else
          tamm_terminate(err_msg + ": Only O,V tiled index spaces can be specified");
      }
    }

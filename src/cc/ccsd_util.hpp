...
struct V2Tensors {
    ...
    std::string v2ijab_file, v2iajb_file, v2ijka_file, v2ijkl_file, v2iabc_file,
      v2abcd_file;
    ...
    void allocate(ExecutionContext& ec, const TiledIndexSpace& MO) {
        ...
        Tensor<T>::allocate(&ec, v2ijab, v2iajb, v2ijka, v2ijkl, v2iabc,
                            v2abcd);
    }
...
void iteration_print(SystemData& sys_data, const ProcGroup& pg, int iter,
                     double residual, double energy, double time) {
    if(pg.rank() == 0) {
        std::cout.width(6);
        ...
        sys_data.results["output"]["CCSD"]["iter"][std::to_string(iter + 1)]
...
std::pair<double, double> rest(ExecutionContext& ec, const TiledIndexSpace& MO,
    ...
    auto l1 = [&]() {
        jacobi(ec, d_r1, d_t1, -1.0 * zshiftl, transpose, p_evl_sorted, noa,
               nob);
    };
    auto l2 = [&]() {
        jacobi(ec, d_r2, d_t2, -2.0 * zshiftl, transpose, p_evl_sorted, noa,
               nob);
    };
...
template<typename T>
std::pair<double, double> rest_cs(
  ExecutionContext& ec, const TiledIndexSpace& MO, Tensor<T>& d_r1,
  ...
  const bool not_spin_orbital = false) {
    ...
    auto l1 = [&]() {
        jacobi_cs(ec, d_r1, d_t1, -1.0 * zshiftl, transpose, p_evl_sorted, noa,
                  nva, not_spin_orbital);
    };
    auto l2 = [&]() {
        jacobi_cs(ec, d_r2, d_t2, -2.0 * zshiftl, transpose, p_evl_sorted, noa,
                  nva, not_spin_orbital);
    };
...
std::tuple<std::vector<T>, Tensor<T>, Tensor<T>, Tensor<T>, Tensor<T>,
           ...
           std::vector<Tensor<T>>, std::vector<Tensor<T>>>
setupTensors(ExecutionContext& ec, TiledIndexSpace& MO, Tensor<T> d_f1,
             int ndiis, bool ccsd_restart = false) {
    ...
    if(rank == 0) {
        ...
        std::cout << " Iter          Residuum       Correlation     Cpu    "
                     "Wall    V2*C2"
                  << std::endl;
        std::cout << std::string(66, '-') << std::endl;
    }
    ...
    return std::make_tuple(p_evl_sorted, d_t1, d_t2, d_r1, d_r2, d_r1s, d_r2s,
                           d_t1s, d_t2s);
}
...
setupTensors_cs(ExecutionContext& ec, TiledIndexSpace& MO, Tensor<T> d_f1,
    ...
    if(!ccsd_restart) {
        for(decltype(ndiis) i = 0; i < ndiis; i++) {
            ...
            d_r2s.push_back(
              Tensor<T>{{v_alpha, v_beta, o_alpha, o_beta}, {2, 2}});
...
void ccsd_stats(ExecutionContext& ec, double hf_energy, double residual,
                double energy, double thresh) {
    auto rank      = ec.pg().rank();
    ...
    if(rank == 0) {
        ...
        if(ccsd_conv) {
            ...
            std::cout << " CCSD correlation energy / hartree =" << std::setw(26)
                      << std::right << energy << std::endl;
            std::cout << " CCSD total energy / hartree       =" << std::setw(26)
...
template<typename T>
Tensor<T> setupV2(ExecutionContext& ec, TiledIndexSpace& MO,
    ...
    // Spin here is defined as spin(p)=spin(r) and spin(q)=spin(s) which is not
    // currently not supported by TAMM.
    ...
    Scheduler{ec}(d_a2(p, q, r, s) =
                    cholVpr(p, r, cindex) * cholVpr(q, s, cindex))(
      d_v2(p, q, r, s) = d_a2(p, q, r, s))(d_v2(p, q, r, s) -= d_a2(p, q, s, r))
      .execute(hw);
...
template<typename T>
std::tuple<Tensor<T>, Tensor<T>, Tensor<T>, TAMM_SIZE, tamm::Tile,
           TiledIndexSpace>
cd_svd_ga_driver(SystemData& sys_data, ExecutionContext& ec,
                 ...
                 bool readv2 = false, std::string cholfile = "",
                 bool is_dlpno = false) {
    ...
    sys_data.n_frozen_virtual =
      sys_data.options_map.ccsd_options.freeze_virtual;
    bool do_freeze =
      sys_data.n_frozen_core > 0 || sys_data.n_frozen_virtual > 0;

    std::string out_fp = sys_data.output_file_prefix + "." +
                         sys_data.options_map.ccsd_options.basis;
    std::string files_dir = out_fp + "_files/" + sys_data.scf_type_string;
    std::string lcaofile  = files_dir + "/" + out_fp + ".lcao";

    if(!readv2) {
        two_index_transform(sys_data, ec, C_AO, F_AO, C_beta_AO, F_beta_AO,
                            d_f1, lcao, is_dlpno);
        if(!is_dlpno)
            cholVpr = cd_svd_ga(sys_data, ec, MO, AO, chol_count, max_cvecs,
                                shells, lcao);
        ...
        if(rank == 0)
            cout << "Number of cholesky vectors to be read = " << chol_count
                 << endl;
    ...
    double cd_svd_time =
      std::chrono::duration_cast<std::chrono::duration<double>>((hf_t2 - hf_t1))
        .count();

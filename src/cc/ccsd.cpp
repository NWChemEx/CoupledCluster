...
using ce_pt     = simde::CorrelationEnergy<simde::type::canonical_reference,
...
TEMPLATED_MODULE_CTOR(CCSD,T) {
    ...
    add_input<double>("printtol")
      .set_default(0.05)
      .set_description(
        "Write T1,T2 amplitudes above a certain threshold to a file");

    add_input<double>("threshold")
      .set_default(1.0e-6)
      .set_description("CCSD Threshold");
    ...
    add_input<bool>("force_tilesize")
      .set_default(false)
      .set_description("Force tilesize specified");

    add_input<int>("itilesize")
      .set_default(1000)
      .set_description("Tilesize for the Cholesky Dimension");

    add_input<int>("ndiis").set_default(5).set_description(
      "number of diis entries");

    add_input<double>("lshift").set_default(0.0).set_description("Level Shift");

    add_input<int>("ccsd_maxiter")
      .set_default(50)
    ...
    add_input<bool>("computeTData")
      .set_default(false)
      .set_description(
        "Compute and write data needed for (T) calculation to disk");

    add_result<tamm::Tensor<T>>("MO Fock Matrix")
            .set_description("The Fock matrix in MO");        
}
...
TEMPLATED_MODULE_RUN(CCSD,T) {
    ...
    auto nwx_shells =
      bra.basis_set().occupied_orbitals().from_space().basis_set();
    ...
    sys_data.n_lindep =
      sys_data.nbf_orig - sys_data.n_occ_alpha - sys_data.n_vir_alpha;
    sys_data.nbf -= sys_data.n_lindep;

    sys_data.options_map.cd_options.diagtol =
      inputs.at("diagtol").value<double>();
    sys_data.options_map.cd_options.max_cvecs_factor =
      inputs.at("max_cvecs_factor").value<int>();
    sys_data.options_map.ccsd_options.debug = inputs.at("debug").value<bool>();
    sys_data.options_map.ccsd_options.printtol =
      inputs.at("printtol").value<double>();
    sys_data.options_map.ccsd_options.threshold =
      inputs.at("threshold").value<double>();
    sys_data.options_map.ccsd_options.force_tilesize =
      inputs.at("force_tilesize").value<bool>();
    sys_data.options_map.ccsd_options.tilesize =
      inputs.at("tilesize").value<int>();
    sys_data.options_map.ccsd_options.itilesize =
      inputs.at("itilesize").value<int>();
    sys_data.options_map.ccsd_options.ndiis = inputs.at("ndiis").value<int>();
    sys_data.options_map.ccsd_options.lshift =
      inputs.at("lshift").value<double>();
    sys_data.options_map.ccsd_options.ccsd_maxiter =
      inputs.at("ccsd_maxiter").value<int>();
    sys_data.options_map.ccsd_options.freeze_core =
      inputs.at("freeze_core").value<int>();
    sys_data.options_map.ccsd_options.freeze_virtual =
      inputs.at("freeze_virtual").value<int>();
    sys_data.options_map.ccsd_options.balance_tiles =
      inputs.at("balance_tiles").value<bool>();
    sys_data.options_map.ccsd_options.profile_ccsd =
      inputs.at("profile_ccsd").value<bool>();
    sys_data.options_map.ccsd_options.writet =
      inputs.at("writet").value<bool>();
    sys_data.options_map.ccsd_options.writev =
      inputs.at("writev").value<bool>();
    sys_data.options_map.ccsd_options.writet_iter =
      inputs.at("writet_iter").value<int>();
    sys_data.options_map.ccsd_options.readt = inputs.at("readt").value<bool>();
    sys_data.options_map.ccsd_options.computeTData =
      inputs.at("computeTData").value<bool>();
    ...
    TiledIndexSpace AO_ortho{IndexSpace{range(0, (size_t)(sys_data.nbf_orig))},
                             ao_ts};

    std::vector<size_t> shell_tile_map; // not used
    tamm::Tensor<T> C_AO{AO_opt, AO_ortho}, F_AO{AO_opt, AO_opt};
    tamm::Tensor<T> C_beta_AO{AO_opt, AO_ortho},
      F_beta_AO{AO_opt, AO_opt}; // not used
    ...
    if(rank == 0) {
        ccsd_options.print();
        std::cout << std::endl
                  << "#occupied, #virtual = " << sys_data.nocc << ", "
                  << sys_data.nvir << std::endl;
    }
    ...
    bool ccsd_restart =
      ccsd_options.readt || ((fs::exists(t1file) && fs::exists(t2file) &&
                              fs::exists(f1file) && fs::exists(v2file)));

    auto [cholVpr, d_f1_, lcao, chol_count, max_cvecs, CI] = cd_svd_ga_driver<T>(
      sys_data, ec, MO, AO_opt, C_AO, F_AO, C_beta_AO, F_beta_AO, li_shells,
    ...
    if(is_rhf)
        std::tie(p_evl_sorted, d_t1, d_t2, d_r1, d_r2, d_r1s, d_r2s, d_t1s,
                 d_t2s) =
          setupTensors_cs(ec, MO, d_f1, ccsd_options.ndiis,
                          ccsd_restart && fs::exists(ccsdstatus) && scf_conv);
    else
        std::tie(p_evl_sorted, d_t1, d_t2, d_r1, d_r2, d_r1s, d_r2s, d_t1s,
                 d_t2s) =
          setupTensors(ec, MO, d_f1, ccsd_options.ndiis,
                       ccsd_restart && fs::exists(ccsdstatus) && scf_conv);
    ...
    if(rank == 0 && debug) {
        ...
        cout << "Eigen values written to file: "
             << files_prefix + ".eigen_values.txt" << endl
             << endl;
    }
    ...
    if(is_rhf)
        std::tie(residual, corr_energy) = cd_ccsd_cs_driver<T>(
          sys_data, ec, MO, CI, d_t1, d_t2, d_f1, d_r1, d_r2, d_r1s, d_r2s,
          d_t1s, d_t2s, p_evl_sorted, cholVpr, ccsd_restart, files_prefix,
          computeTData);
    ...
    ccsd_stats(ec, sys_data.scf_energy, residual, corr_energy,
               ccsd_options.threshold);
    ...
    double ccsd_time =
      std::chrono::duration_cast<std::chrono::duration<double>>((cc_t2 - cc_t1))
        .count();
    if(rank == 0) {
        if(is_rhf)
            std::cout << std::endl
                      << "Time taken for Closed Shell Cholesky CCSD: "
                      << ccsd_time << " secs" << std::endl;
        else
            std::cout << std::endl
                      << "Time taken for Open Shell Cholesky CCSD: "
                      << ccsd_time << " secs" << std::endl;
    }
    ...
    if(rank == 0 && debug) {
        std::cout << std::endl
                  << "Threshold for printing amplitudes set to: " << printtol
                  << std::endl;
        std::cout << "T1, T2 amplitudes written to files: "
                  << files_prefix + ".print_t1amp.txt"
                  << ", " << files_prefix + ".print_t2amp.txt" << std::endl
                  << std::endl;
        print_max_above_threshold(d_t1, printtol,
                                  files_prefix + ".print_t1amp.txt");
        print_max_above_threshold(d_t2, printtol,
                                  files_prefix + ".print_t2amp.txt");
    }
